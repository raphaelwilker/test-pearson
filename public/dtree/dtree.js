...
// ----------------------------------------------------------------------------
	...
	this.preloadIcons = function()
	{
		...
		this.arrIcons[4].src = this.imgFolder + 'folder-tree.png';
		this.arrIcons[5] = new Image();
		this.arrIcons[5].src = this.imgFolder + 'folder-tree-open.png';
	}
			...
			if (this.arrNodes[n].pid == pNode)
			{
				...
				if (cn.url)
				{
					this.strOutput += '<a href="' + cn.url + '"';
                                        this.strOutput += (cn.img) ? cn.img : ((this.rootNode == cn.pid) ? '' : (cn._hc) ? ((cn._io) ? '' : '') : 'class=\'file\'');
				...
				this.strOutput += (cn.img) ? cn.img : ((this.rootNode == cn.pid) ? 'base' : (cn._hc) ? ((cn._io) ? 'folder-tree-open' : 'folder-tree') : 'file-tree') + '.png';
                                this.strOutput += '" alt="" />';
                        }
